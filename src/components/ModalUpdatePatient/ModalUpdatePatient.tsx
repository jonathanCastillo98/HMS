...
const ModalUpdatePatient = ({ setModalState, onRefresh, name, id }: Props) => {
  ...
  useEffect(() => {
    ...
    const updateDoctor = async (displayName: string, email: string, password: string) => {
      let data = {
      ...
      try {
        const req = await fetch(`http://localhost:3000/admin/patients/${id}`, {
          method: 'PUT',
    ...
    updateDoctor(inputs.displayName, inputs.email, inputs.password)
  }, [inputs])

  const { register, handleSubmit, formState: { errors } } = useForm({ defaultValues: { displayName: name, email: "", password: "" } });
